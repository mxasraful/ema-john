import React from 'react';
import './Login.css'
import Auth from "./useAuth";
import Button from '@material-ui/core/Button';
import { useState } from 'react';


const Login = () => {

    const [name, setName] = useState('')
    const [email, setEmail] = useState('')
    const [pass, setPass] = useState('')

    const [signIn, setSignIn] = useState(true)

    const auth = Auth();

    // Handle Google Signin
    const HandelSingInWithGoogle = () => {
        auth.singInWithGoogle()
    }
    
    // Handle Password Sigup
    const handelSignUp = (e) => {
        auth.signUpWithPass(name, email, pass)
        e.preventDefault()
        e.target.reset()
    }

    // Handle Password Sigin
    const handelSignIn = (e) => {
        auth.signInWithPass(email, pass)
        e.preventDefault()
        e.target.reset()
    }


    return (
        <div>
            {
                auth.user ?
                    <div className='isSignedInSection'>
                        <h2>You are signed in</h2>
                    </div>
                    :
                    <div className="signUpContainer">
                        <Button className="clickToSignInWithGoogle" onClick={HandelSingInWithGoogle}><img className='loginOptionImg' src="data:image/svg+xml;base64," alt="" /><span>Sign In With Google</span></Button>
                        <Button className="clickToSignInWithGoogle" onClick={auth.signedInWithFb}><img src="data:image/svg+xml;base64," alt="" className="loginOptionImg" /><span>Sign In with Facebook</span></Button>
                        <br />
                        {
                            signIn ?
                                <>
                                    <h2>Sign In</h2>
                                    <br />
                                    <form onSubmit={handelSignIn} action="" className="signInForm">
                                        <input onChange={(e) => setEmail(e.target.value)} type="email" className="inputForm inputEmail" placeholder="Your Email" name="name" required /><br />
                                        <input onChange={(e) => setPass(e.target.value)} type="password" className="inputForm inputPassword" placeholder="Your Password" name="name" required /><br />
                                        <div className="submitBtbSection" style={{ display: "flex" }}><Button id="createAccountBtn" type="submit" className="submit" value="">Sign Up</Button><p></p></div>
                                        <div onClick={() => setSignIn(false)} className='signInTrueFalse'>I Don't have an account</div>
                                        <br />
                                        <Button className='forgotPBtn' style={{ marginLeft: '115px' }} href='./../forgotpassword' variant="contained" color="primary">Forgot Password</Button>
                                        <br />
                                        <br />
                                    </form>
                                </> :
                                <>
                                    <h2>Create Account</h2>
                                    <br />
                                    <form onSubmit={handelSignUp} action="" className="signInForm">
                                        <input onChange={(e) => setName(e.target.value)} type="name" className="inputForm inputName" placeholder="Your Name" name="name" required /><br />
                                        <input onChange={(e) => setEmail(e.target.value)} type="email" className="inputForm inputEmail" placeholder="Your Email" name="name" required /><br />
                                        <input onChange={(e) => setPass(e.target.value)} type="password" className="inputForm inputPassword" placeholder="Your Password" name="name" required /><br />
                                        <div className="submitBtbSection" style={{ display: "flex" }}><Button id="createAccountBtn" type="submit" className="submit" value="">Sign Up</Button><p></p></div>
                                        <div style={{ width: '125px' }} onClick={() => setSignIn(true)} className='signInTrueFalse'>I have an account</div>
                                        <br /><br />
                                    </form>
                                </>
                        }
                    </div>}
        </div>
    );
};

export default Login;